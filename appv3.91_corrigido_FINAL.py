...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Cyber Threat Research - Caçador de Ameaças V3.9",
    page_icon="🛡️",
    layout="wide",
...
# AUTENTICAÇÃO SEGURA
# -----------------------------------------------------------------------------
# As credenciais devem ser armazenadas exclusivamente como hashes Argon2 em:
#
# .streamlit/secrets.toml
#
# [credentials]
# root = "$argon2id$v=19$m=65536,t=3,p=4$..."
#
# Não existe senha padrão ou fallback no código-fonte.
...
def get_dummy_password_hash():
    """
    Hash utilizado para processar tentativas com usuários inexistentes.

    Isso reduz diferenças perceptíveis no tempo de resposta entre um usuário
    existente e um usuário inexistente.
    """
    return get_password_hasher().hash(
        "dummy-password-not-valid-for-authentication"
    )


def get_user_credentials():
    """
    Lê usuários e hashes Argon2 do st.secrets.

    Nenhuma credencial padrão é mantida no código.
    """
    try:
        credentials = st.secrets.get("credentials", {})
        return {
            str(username).strip(): str(password_hash).strip()
            for username, password_hash in dict(credentials).items()
...
def verify_password(password, stored_hash):
    ...
    Retorna False para senha incorreta, hash inválido ou qualquer falha
    de verificação, sem expor detalhes na interface.
    """
    try:
        return get_password_hasher().verify(stored_hash, password)
...
def logout():
    """
    Encerra a autenticação e remove informações sensíveis da sessão.
    """
    sensitive_keys = [
        "active_vt_key",
        "active_abuse_key",
...
def render_login_header():
    st.markdown(
        ...
        <div style="text-align: center; padding: 2rem 1rem;">
            ...
            ">
                🛡️ Cyber Threat Research - Threat Intel
            </h1>

            <p style="color: #94a3b8; font-size: 1.1rem;">
                Acesse o sistema em
                <a
                    href="https://ctrdefense.io"
                    target="_blank"
...
def render_responsibility_disclaimer():
    st.markdown(
        """
        <div style="
        ...
        ">
            ...
            <p style="margin: 0 0 10px 0;">
                O usuário é integralmente responsável pelas consultas
                realizadas e pelo uso dos resultados obtidos.
            </p>

            <p style="margin: 0 0 10px 0;">
                As informações fornecidas pelas fontes de inteligência podem
                estar <strong>incompletas, desatualizadas, incorretas ou
                sujeitas a alterações</strong>. Os resultados devem ser
                tratados como informações de apoio e, quando necessário,
                validados por fontes oficiais ou pelos responsáveis pelos
                ativos.
            </p>

            <p style="margin: 0 0 6px 0;">
                <strong>O desenvolvedor da ferramenta não se responsabiliza por:</strong>
            </p>
            ...
            <ul style="margin: 0; padding-left: 22px;">
                ...
                <li>Ações realizadas pelo usuário com base nas informações apresentadas.</li>
            </ul>
        </div>
        """,
        unsafe_allow_html=True,
...
def check_password():
    """
    Autenticação segura usando Argon2 e Streamlit Secrets.

    Espera no .streamlit/secrets.toml:

        [credentials]
        admin = "$argon2id$..."

    Não existe senha padrão no código-fonte.
    """
    ...
    render_login_header()
    render_responsibility_disclaimer()
    ...
    if not credentials:
        st.error(
            ...
            "Cadastre pelo menos um usuário em "
            "`.streamlit/secrets.toml`."
        )
        st.info(
            "No Streamlit Cloud, configure a seção [credentials] em "
            "Settings → Secrets."
        )
        st.stop()
    ...
    locked = (
        time.time() < st.session_state.lockout_until
    )

    if locked:
        remaining = max(
            1,
            int(st.session_state.lockout_until - time.time()),
        )
        st.warning(
            f"Login temporariamente bloqueado. "
            f"Tente novamente em aproximadamente {remaining}s."
        )
        return False
    ...
    # Hash dummy mantém o custo computacional semelhante para usuário
    # inexistente e evita diferenças triviais de tempo.
    stored_hash = credentials.get(normalized_username)

    if stored_hash:
        candidate_hash = stored_hash
    else:
        candidate_hash = get_dummy_password_hash()

    password_ok = False

    try:
        password_ok = get_password_hasher().verify(
            candidate_hash,
            password,
        )
    except (
    ...
    valid_user = normalized_username in credentials
    ...
    if st.session_state.login_attempts >= MAX_LOGIN_ATTEMPTS:
        ...
        st.error(
            f"Limite de tentativas excedido. "
            f"Login bloqueado por {LOGIN_LOCKOUT_SECONDS} segundos."
        )
    else:
        remaining = (
            MAX_LOGIN_ATTEMPTS
