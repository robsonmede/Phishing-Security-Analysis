...
with tab_greynoise:
    ...
    if not GREYNOISE_API_KEY:
        st.info("ℹ️ **Modo Comunitário Público Ativo:** Consultando via `https://api.greynoise.io/v3/community/{ip}` sem chave de API (com limites por IP).")
    else:
        ...
        if gn_ip:
            with st.spinner("Consultando GreyNoise..."):
                ...
                if "error" in gn_res:
                    ...
                    col1.metric("Classificação", gn_res.get("classification", "Desconhecido").upper())
                    ...
                    st.markdown(f"**Nome da Entidade / Botnet:** `{gn_res.get('name', 'N/A')}`")
                    st.markdown(f"**Última Detecção:** `{gn_res.get('last_seen', 'N/A')}`")
                    
                    if gn_res.get("link"):
                        st.markdown(f"🔗 [Visualizar detalhes no GreyNoise Viz]({gn_res.get('link')})")
