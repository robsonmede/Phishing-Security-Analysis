...
def search_attackerkb(query):
    ...
    try:
        res = requests.get("https://api.attackerkb.com/v1/topics", headers={"X-API-Key": ATTACKERKB_API_KEY, "Accept": "application/json"}, params={"search": query, "limit": 10}, timeout=15)
        if res.status_code == 200:
            return res.json()
        if res.status_code == 401:
            return {"error": "Chave AttackerKB inválida."}
        if res.status_code == 429:
...
def render_tool_osint_card(name, category, mode, description, url, requirements):
    st.markdown(f"### {name}")
...
with tab_osint:
    ...
    st.caption("Catálogo operacional de ferramentas de Threat Hunting, DFIR, malware analysis e OSINT.")
    st.info("Ferramentas locais não são tratadas como APIs públicas. A aba mostra a finalidade, o tipo de integração e os requisitos; as integrações HTTP opcionais ficam restritas a AttackerKB, BotScout e instâncias próprias de Cuckoo/theHarvester.")
    left, right = st.columns(2)
    with left:
        render_tool_osint_card("🕵️ APT-Hunter", "Threat Hunting / DFIR", "Local / EVTX", "Analisa Windows Event Logs/EVTX com regras de detecção e estatísticas para evidenciar movimentos APT e produzir timelines.", "https://github.com/ahmedkhlief/APT-Hunter", "Python 3 + EVTX")
        render_tool_osint_card("🧪 DeepBlueCLI", "Threat Hunting", "Local / PowerShell", "Hunting sobre Windows Security, System, Application, PowerShell e Sysmon, incluindo obfuscação, password spraying e comportamento suspeito.", "https://github.com/sans-blue-team/DeepBlueCLI", "PowerShell + logs")
        render_tool_osint_card("🧰 Automater", "OSINT / IOC Enrichment", "Local", "Ferramenta para enriquecer URL, domínio, IP e hash usando fontes de OSINT disponíveis.", "https://github.com/1N3/Automater", "Execução local")
        render_tool_osint_card("🎣 Phishing Catcher", "Phishing / CT", "Local / CertStream", "Monitora emissões de certificados em Certificate Transparency e pontua domínios potencialmente ligados a phishing.", "https://github.com/x0rz/phishing_catcher", "Python + CertStream")
    with right:
        render_tool_osint_card("🌐 theHarvester", "OSINT", "CLI + REST local", "Coleta informações de fontes OSINT selecionadas, incluindo hosts, subdomínios e emails; a API REST local pode ser protegida por chave do servidor.", "https://github.com/laramies/theHarvester", "Fontes configuradas")
        render_tool_osint_card("🦠 Cuckoo Sandbox", "Malware Analysis", "Instância própria", "Sandbox de análise dinâmica. A aplicação apenas verifica conectividade da instância configurada e não submete amostras automaticamente.", "https://cuckoosandbox.org/", "API HTTP local")
        render_tool_osint_card("📦 CrowdFMS", "Threat Research", "Local + VT Private API", "Automatiza coleta/processamento de amostras relacionadas a alertas YARA no VirusTotal; depende do Private API.", "https://github.com/CrowdStrike/CrowdFMS", "VT Private API + YARA")
        render_tool_osint_card("🎯 AttackerKB", "Vulnerability Intelligence", "REST API", "Base de conhecimento de vulnerabilidades, avaliações e contexto de exploração.", "https://api.attackerkb.com/v1/api-docs/docs", "API Key")
    st.divider()
    st.subheader("🔌 Status das integrações opcionais")
    c1, c2 = st.columns(2)
    with c1:
...
with tab_cross_intel:
    ...
    st.text_input("CVE / termo opcional para AttackerKB:", placeholder="Ex.: CVE-2024-21410", key="cross_attackerkb_query")
