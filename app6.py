...
# Leitura segura da chave através do st.secrets
VT_API_KEY = st.secrets.get("VIRUSTOTAL_API_KEY", "")

# Barra superior de status de conexão com a API
...
# -----------------------------------------------------------------------------
# FUNÇÕES DE COMUNICAÇÃO COM A API V3 DO VIRUSTOTAL
# -----------------------------------------------------------------------------
def get_vt_data(endpoint, item_id):
    """Realiza a requisição na API v3 do VirusTotal utilizando a chave informada."""
    if not VT_API_KEY or VT_API_KEY == "sua_chave_api_do_virustotal_aqui":
        return {"error": "Chave API não configurada em Secrets"}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"error": "Chave API Inválida/Não Autorizada"}
        elif response.status_code == 429:
            return {"error": "Limite de requisições excedido (Quota Exceeded)"}
        else:
...
def parse_vt_stats(vt_response):
    """Interpreta e formata a resposta obtida do VirusTotal."""
                ...
                with st.spinner("Consultando reputação na API do VirusTotal..."):
                    for h in all_hashes:
                        ...
                        hash_data.append({
                            ...
                            "Reputação VirusTotal (API)": parse_vt_stats(vt_resp),
