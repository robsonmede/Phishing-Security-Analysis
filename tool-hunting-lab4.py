...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Cyber Threat Research - Caçador de Ameaças V1.3",
    page_icon="🛡️",
    layout="wide",
    ...
    with col_link3:
        st.markdown("""
            <a href="https://www.hybrid-analysis.com/" target="_blank" class="tool-card">
                ...
                <div class="tool-desc">Análise automatizada gratuita de malware e sandbox.</div>
            </a>
        """, unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# 6. MÓDULO VIRUSTOTAL API V3 (ATUALIZADO PARA EXTRAÇÃO AVANÇADA DE HASHES)
# -----------------------------------------------------------------------------
...
def parse_vt_details(vt_response):
    ...
    try:
        ...
        # Extração de Propriedades Básicas e Nomes de Arquivo
        ...
        return {
            ...
            "detalhes": f"Malicioso: {malicious}", 
...
with tab_iocs:
    ...
    if st.button("Executar Triagem do Artefato", type="primary"):
        ...
        if all_hashes:
            st.subheader("🧩 Hashes de Artefatos & Propriedades Básicas")
            hash_data = []
            for h in all_hashes:
                vt_info = parse_vt_details(get_vt_data("files", h))
                hash_data.append({
                    "Hash": h,
                    "Nome do Arquivo": vt_info["file_name"],
                    ...
                    "Link Hybrid Analysis": f"https://www.hybrid-analysis.com/search?query={h}"
                })
            st.dataframe(
                pd.DataFrame(hash_data), 
                column_config={
                    "Link VirusTotal": st.column_config.LinkColumn("VirusTotal", display_text="Abrir VT ↗"),
                    "Link Hybrid Analysis": st.column_config.LinkColumn("Hybrid Analysis", display_text="Abrir HA ↗")
                }, 
...
# =============================================================================
# ABA 4: CALCULADOR DE ENTROPIA DE SHANNON
# =============================================================================
with tab_entropy:
    ...
    st.info(
        ...
        "A **calculadora de entropia de Shannon** serve para medir o grau de incerteza, surpresa ou quantidade de informação contida em um conjunto de dados ou em uma fonte de probabilidades.\n\n"
        "* **Incerteza alta (Entropia alta):** Se todos os eventos têm a mesma chance de acontecer, a incerteza é alta e a entropia é alta.\n"
        "* **Incerteza mínima (Zero):** Se um evento tem 100% de certeza de ocorrer, não há surpresa e a entropia é zero.\n\n"
        "---"
        "🎯 **Principais Aplicações em Cibersegurança:**\n\n"
        "* **Detecção de Malware:** Identificação de seções executáveis (PE/ELF) empacotadas (packed) ou obfuscadas. Malwares usam obfuscação para ocultar código, elevando a entropia.\n"
        "* **Esteganografia:** Identificação de dados ocultos dentro de mídias (como imagens PNG/JPG e arquivos áudio). O payload escondido altera a distribuição dos bytes do arquivo original, elevando a entropia da seção afetada.\n"
        "* **Teste de Criptografia:** Validação da aleatoriedade de geradores de números pseudoaleatórios (PRNG), chaves criptográficas e cifras de fluxo (stream ciphers).\n\n"
        "---"
        "🔑 **Exemplos de Chaves / Faixas de Entropia (Escala de 0.0 a 8.0):**\n\n"
        "* `0.0 a 3.0` → **Entropia Muito Baixa:** Texto simples com alta repetição de caracteres (Ex: `AAAAAA` ou logs redundantes).\n"
        "* `3.1 a 5.8` → **Entropia Baixa / Normal:** Código-fonte legível (C, Python, HTML), texto em linguagem natural ou executáveis compilados não empacotados.\n"
        "* `5.9 a 7.0` → **Entropia Moderada:** Arquivos compactados leves, scripts obfuscados de baixa complexidade ou dados codificados em Base64.\n"
        "* `7.1 a 7.8` → **Entropia Alta (Ameaça Suspeita):** Arquivos executáveis empacotados (UPX, Themida), dados com payload esteganográfico ou arquivos comprimidos (ZIP, RAR, 7z).\n"
        "* `7.9 a 8.0` → **Entropia Máxima (Aleatoriedade Puramente Criptográfica):** Chaves AES/RSA puras, fluxos de dados criptografados (TLS/VPN) e geradores de ruído aleatório."
    )
...
st.markdown("""
    <div class="footer-text">
        CTRDEFENSE.BLOG &copy; 2026 | Cyber Threat Research - Caçador de Ameaças V1.3 (Threat Hunting)
    </div>
""", unsafe_allow_html=True)
