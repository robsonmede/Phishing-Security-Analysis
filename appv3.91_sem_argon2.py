...
def check_password():
    """
    Interrompe a aplicação enquanto o usuário não estiver autenticado.
    """
    ...
    remaining_lockout = max(
        0,
        int(st.session_state.login_locked_until - now),
    )
    locked = remaining_lockout > 0

    if locked:
        st.error(
            "🔒 Muitas tentativas inválidas. "
            f"Tente novamente em {remaining_lockout} segundo(s)."
        )
