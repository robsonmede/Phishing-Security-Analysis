...
with st.sidebar:
    ...
    user_greynoise_key = st.text_input("GreyNoise API Key (Opcional):", value=st.session_state.get("greynoise_key_input", def_greynoise), type="password", key="greynoise_key_input")
...
status_cols[3].caption("🟢 GreyNoise (Auth)" if GREYNOISE_API_KEY else "🟡 GreyNoise (Public)")
...
# --- GreyNoise (Suporta modo Autenticado e Comunitário Sem Autenticação) ---
def check_greynoise(ip_address):
    url = f"https://api.greynoise.io/v3/community/{ip_address}"
    headers = {"Accept": "application/json"}
    
    # Se houver chave informada, inclui no cabeçalho
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            return {"message": "IP não catalogado como ruído generalizado (Não encontrado)."}
        elif res.status_code == 429:
            return {"error": "Limite de requisições do plano comunitário atingido."}
...
# =============================================================================
# ABA 5: GREYNOISE (COMMUNITY UNAUTHENTICATED / AUTH)
# =============================================================================
with tab_greynoise:
    ...
    if not GREYNOISE_API_KEY:
        st.info("ℹ️ **Modo Comunitário Público Ativo:** Consultando via `https://api.greynoise.io/v3/community/{ip}` sem chave de API.")
    else:
        st.success("🔑 **Modo Autenticado Ativo:** Utilizando chave de API cadastrada.")
        ...
        if gn_ip:
            with st.spinner("Consultando GreyNoise..."):
                ...
                if "error" in gn_res:
                    ...
                    # Classificação, Noise e RIOT
                    col1.metric("Classificação", str(gn_res.get("classification", "Desconhecido")).upper())
                    col2.metric("Noise (Varredura Geral)", "Sim" if gn_res.get("noise") else "Não")
                    col3.metric("RIOT (Serviço Confiável)", "Sim" if gn_res.get("riot") else "Não")

                    st.markdown("### 📋 Detalhes do Endereço IP")
                    
                    # Mapeamento de dados (preparado para lidar com o limite do endpoint comunitário)
                    # No endpoint comunitário, "name" geralmente traz o ator ou a organização.
                    actor = gn_res.get("actor", gn_res.get("name", "N/D"))
                    organization = gn_res.get("organization", gn_res.get("name", "N/D"))
                    asn = gn_res.get("asn", "N/D (Requer API Context)")
                    last_seen = gn_res.get("last_seen", "Desconhecido")
                    first_seen = gn_res.get("first_seen", "")
                    
                    # Formatação da Timeline
                    timeline_str = f"Última detecção (Last Seen): {last_seen}"
                    if first_seen:
                        timeline_str = f"Primeira detecção: {first_seen} | " + timeline_str

                    # Exibição dos atributos
                    st.markdown(f"**👤 Actor / Nome:** `{actor}`")
                    st.markdown(f"**🏢 Organização:** `{organization}`")
                    st.markdown(f"**🌐 ASN:** `{asn}`")
                    st.markdown(f"**⏳ TimeLine:** `{timeline_str}`")
                    
                    # Verificação de Tags (Comum em contas pesquisador/Enterprise)
                    tags = gn_res.get("tags", [])
                    if tags:
                        st.markdown("**🏷️ Tags Associadas:**")
                        # Cria badges visuais para as tags
                        tags_html = "".join([f"<span style='background-color:#1e293b; border: 1px solid #38bdf8; padding: 4px 8px; border-radius: 4px; margin-right: 6px; font-family: monospace; font-size: 0.85em;'>{t}</span>" for t in tags])
                        st.markdown(tags_html, unsafe_allow_html=True)
                    else:
                        st.markdown("**🏷️ Tags:** `N/A (Sem tags informadas ou endpoint não suporta)`")

                    st.write("---")
                    if gn_res.get("link"):
                        st.markdown(f"🔗 [Visualizar análise completa no GreyNoise Viz]({gn_res.get('link')})")
