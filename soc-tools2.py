...
with st.sidebar:
    ...
    # Busca chave padrão do secrets.toml se configurada
    default_key = st.secrets.get("VIRUSTOTAL_API_KEY", "")

    # Campo mascarado (password) para entrada manual via UI
    user_vt_key = st.text_input(
        "VirusTotal API Key:",
        value=st.session_state.get("vt_key_input", default_key),
        type="password",
        help="Insira sua chave API do VirusTotal v3. Se configurada no secrets.toml, ela será carregada automaticamente.",
        key="vt_key_input"
    )
...
# Prioriza a chave ativa da sessão, a chave digitada na UI e, por fim, o secrets.toml
...
# Estado compartilhado para análise comportamental
...
# =============================================================================
# ABA 2: MAPEADOR DINÂMICO MITRE ATT&CK
# =============================================================================
with tab_mitre:
    ...
    if not target_text.strip():
        ...
        if detected_ttps:
            ...
            st.warning("Nenhum padrão comportamental evidente mapeado na base atual do MITRE para o texto informado.")
