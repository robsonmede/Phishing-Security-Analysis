            ...
            if kind not in ("IP", "DOMAIN", "URL"):
                ...
                with st.spinner("Consultando fontes públicas de enriquecimento..."):
                    ...
                    if kind == "IP":
                        ...
                        results["crt.sh"] = check_crtsh(domain)
                ...
                # Seção: Certificates
                st.markdown("### 📜 Certificados (crt.sh)")
                ...
                # Não temos fonte pública direta para técnicas por IP/URL; exibimos placeholder se VT fornecer popular_threat_classification
                mitre_techniques = []
                if "VirusTotal" in results and isinstance(results["VirusTotal"], dict):
                    vt = results["VirusTotal"]
                    ptc = vt.get("popular_threat_classification", {})  # não é extraído atualmente
                    if ptc:
