...
# --- XposedOrNot (Sem necessidade de API Key) ---
def check_xposedornot_account(email):
...
# =============================================================================
# ABA 7: ANALISADOR DE E-MAIL & XPOSEDORNOT
# =============================================================================
with tab_email:
    st.header("📧 Analisador de E-mail & XposedOrNot")
    ...
    with col_em2:
        st.subheader("2. Checar Vazamentos (XposedOrNot - Free)")
        target_email = st.text_input("E-mail para consulta na XposedOrNot:", placeholder="vitima@empresa.com")
        if st.button("Verificar Vazamentos"):
            if target_email:
                xon_res = check_xposedornot_account(target_email)
                if isinstance(xon_res, dict):
                    if "error" in xon_res:
                        st.warning(f"Erro/Status: {xon_res['error']}")
                    elif xon_res.get("status") == "clean" or not xon_res.get("breaches"):
                        st.success("✅ Nenhum vazamento registrado para esta conta.")
                    else:
                        breaches = xon_res.get("breaches", [])
                        if isinstance(breaches, list) and len(breaches) > 0 and isinstance(breaches[0], list):
                            breaches = breaches[0]
                        st.error(f"🚨 Encontrado em {len(breaches)} vazamentos!")
                        st.write("**Vazamentos Detectados:**")
                        for b in breaches:
                            st.write(f"- {b}")
                else:
                    st.warning(f"Resposta inesperada da API: {xon_res}")
